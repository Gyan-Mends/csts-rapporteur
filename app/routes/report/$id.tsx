...
export const loader: LoaderFunction = async ({ params }) => {
  try {
    const response = await fetch(`http://localhost:5173/api/reports/${params.id}`);
    ...
    if (!data.success) {
      throw new Response(data.message || "Failed to fetch report", { status: 400 });
    }
            ...
            {report.fileUrl ? (
              <div className="flex flex-col sm:flex-row gap-4 justify-center">
                <button
                  ...
                  className="inline-flex items-center justify-center gap-2 bg-white text-pink-600 hover:bg-gray-50 font-semibold px-8 py-4 rounded-lg transition-colors duration-200 border border-pink-200"
                >
